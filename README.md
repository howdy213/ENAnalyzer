...
- 自动打包成功后，下方会弹出提示窗口，可在 7 秒内**撤销**该次打包，文件将被删除并标记为跳过。
- 提示窗口可选关闭
- 可以设置提示窗口距底部的位置

### 🧹 Dependencies 目录清理

- 监视希沃白板缓存目录（`%APPDATA%\Seewo\EasiNote5\Dependencies`），可定期自动删除指定子目录，释放磁盘空间。
- 支持添加/取消监视、删除目录，方便管理缓存数据。

### 🚫 自动关闭 EasiNote 窗口

- 可配置自动关闭 EasiNote 进程产生的窗口。
- 支持按窗口**标题**匹配。
- 内置常用标题，也可手动添加自定义标题。
- 定时扫描并关闭匹配的子窗口，保持教学环境整洁。
...
此外，**功能设置**标签页拥有独立的配置文件（`config/function_settings.json`），用于保存 Dependencies 监视列表、自动关闭弹窗标题及启用状态等，可通过界面进行管理。
...
| **功能设置** | Dependencies 目录监视与管理、自动关闭 EasiNote 弹窗设置   |
| **关于**     | 版本信息、项目主页、功能列表                              |
